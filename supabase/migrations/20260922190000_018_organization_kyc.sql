...
alter table public.organizations drop constraint if exists organizations_kyc_status_check;
alter table public.organizations add constraint organizations_kyc_status_check
  check (kyc_status in ('none','pending','verified','rejected'));
alter table public.organizations drop constraint if exists organizations_kyc_document_type_check;
alter table public.organizations add constraint organizations_kyc_document_type_check
...
create unique index if not exists kyc_verifications_org_external_id_uq on public.kyc_verifications(organization_id, external_id) where external_id is not null;
create index if not exists kyc_verifications_org_created_idx on public.kyc_verifications(organization_id, created_at desc);
create index if not exists organizations_kyc_required_idx on public.organizations(kyc_required) where kyc_required = true;

alter table public.kyc_verifications enable row level security;
drop policy if exists "kyc_verifications_select_own_org" on public.kyc_verifications;
create policy "kyc_verifications_select_own_org" on public.kyc_verifications
